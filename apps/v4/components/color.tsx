...
export function Color({ color }: { color: Color }) {
  ...
  return (
    <button
      key={color.hex}
      className="text-(--text) group relative flex aspect-[3/1] w-full flex-1 cursor-pointer flex-col gap-2 sm:aspect-[2/3] sm:h-auto sm:w-auto [&>svg]:absolute [&>svg]:right-4 [&>svg]:top-4 [&>svg]:z-10 [&>svg]:h-3.5 [&>svg]:w-3.5 [&>svg]:opacity-0 [&>svg]:transition-opacity"
    ...
    >
      ...
      <div className="border-ghost after:border-input bg-(--bg) w-full flex-1 rounded-md after:rounded-lg md:rounded-lg" />
