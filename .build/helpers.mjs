...
export const types = ['linear', 'bold'];
  ...
  types.forEach((type) => {
    icons[type] = globSync(slash(path.join(ICONS_SRC_DIR, `${type}/*.svg`)))
      ...
      .map((i) => {
        ...
        return {
          name,
          namePascal: toPascalCase(`${name} icon`),
          path: i,
...
export const getAllIconsMerged = (withContent = false, withObject = false) => {
  ...
  allIcons.bold.forEach((icon) => {
    if (icons[icon.name]) {
      icons[icon.name].styles.bold = {
        version: icon.version || '',
        unicode: icon.unicode || '',
        ...(withContent ? { content: icon.content } : {}),
        ...(withObject ? { obj: icon.obj } : {}),
...
export const getAliases = (groupped = false) => {
  const allAliases = JSON.parse(fs.readFileSync(resolve(HOME_DIR, 'icons-aliases.json'), 'utf-8'));
  ...
  if (groupped) {
    ...
    types.forEach((type) => {
      ...
      for (const [key, value] of Object.entries(allAliases[type])) {
        if (icons.includes(value)) {
          aliases[`${key}${type !== 'linear' ? `-${type}` : ''}`] =
            `${value}${type !== 'linear' ? `-${type}` : ''}`;
        }
      }
    });
